    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userPath.equals("/admin/agregarProducto")) {
            ...
            try{
                ...
                String idSeleccionado = request.getParameter("categoriaSeleccionada").toString();
                producto.setIdProducto(productoFacade.sequenceVal());
                producto.setNombreProducto(nombreProducto);
                producto.setPrecioProducto(precioProducto);
                producto.setDescripcionProducto(descripcionProducto);
                System.out.println(producto.getIdProducto() + " " +idSeleccionado);
                productoFacade.create(producto);
                ProductoCategoria pc= new ProductoCategoria();
                ProductoCategoriaPK pcPK= new ProductoCategoriaPK();
                Categoria categoriaSearch = categoriaFacade.find(Integer.parseInt(idSeleccionado));
                
                pcPK.setPkFkCategoria(categoriaSearch.getIdCategoria());
                pcPK.setPkFkProducto(producto.getIdProducto());
                pcPK.setIdProductoCategoria(productoCategoriaFacade.sequenceVal());
                
                pc.setCategoria(categoriaSearch);
                pc.setProducto(producto);
                pc.setProductoCategoriaPK(pcPK);
                productoCategoriaFacade.create(pc);
